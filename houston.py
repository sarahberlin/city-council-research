...
def get_councilor_data(page_url):
    ...
    soup = bs4.BeautifulSoup(response.text)
    try:
        councilor_data['district'] = soup.select('h1')[0].get_text().encode('utf-8').replace('City ','')
        councilor_data['name'] = soup.select('p span.contBold')[0].get_text().replace('Council Member ', '')   
