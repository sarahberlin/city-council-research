...
for x in range(0,9):
    councilor_data = {}
    try:
        councilor_data['district'] = soup.select('div.cdinfo')[x].get_text().replace('District 2 ','District 2 \r\n\t\t').replace('District 8 ','District 8 \r\n\t\t').replace('\r\n\t\r\n\t','\r\n\t').replace('\r\n\t', ',').replace('\t   ', '').replace('\t', '').replace('\r\n                ',',').replace("Email: ", "").replace('\n', '').encode('utf-8').split(',')[0]
        councilor_data['name'] = soup.select('div.cdinfo')[x].get_text().replace('District 2 ','District 2 \r\n\t\t').replace('District 8 ','District 8 \r\n\t\t').replace('\r\n\t\r\n\t','\r\n\t').replace('\r\n\t', ',').replace('\t   ', '').replace('\t', '').replace('\r\n                ',',').replace("Email: ", "").replace('\n', '').encode('utf-8').split(',')[1]
...
#creates csv
fieldnames = ['district', 'name', 'website','email', 'address', 'phone']
