...
def get_councilor_data(page_url):
    ...
    try:        
        if page_url == '/government/citycouncil/district6/':
            councilor_data['district'] = soup.select('h1 span')[1].get_text().encode('utf-8').replace('\xe2\x80\x8b', '')
            councilor_data['name'] = soup.select('h1 span')[0].get_text().encode('utf-8').replace('\xe2\x80\x8b\xc2\xa0', '')
            councilor_data['website'] =  root_url + page_url
        elif page_url == '/government/citycouncil/district14/':
            councilor_data['district'] = soup.select('div h1')[3].get_text().encode('utf-8')
            councilor_data['name'] =soup.select('div h1')[2].get_text().encode('utf-8')
            councilor_data['website'] =  root_url + page_url
        else:
            ...
            councilor_data['district'] = soup.select('h1 p')[1].get_text().encode('utf-8').replace('\xe2\x80\x8b', '')
            councilor_data['name'] = soup.select('h1 p')[0].get_text().encode('utf-8').replace('\xe2\x80\x8b', '').replace('\xc2\xa0', '')
            councilor_data['website'] =  root_url + page_url
